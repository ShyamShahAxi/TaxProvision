...
function loadSample() {
  ...
  const L = (label, amount, type, source) => ({ id: uid(), label, amount, type: type || 'permanent', source: source || 'manual' });
  ...
  provision = Object.assign(emptyProvision(), {
    ...
    addBacks: [
      // Permanent differences (Section B)
      L('Keyman / group insurance premiums (non-deductible)', 25919, 'permanent'),
      L('Staff medical expenses over 1% cap', 67287, 'permanent'),
      // Temporary differences — book charges added back (Section C)
      L('Depreciation — Computer Equipment', 78039, 'temporary'),
      L('Amortisation — Development costs', 459311, 'temporary'),
      L('Leave provision — closing balance', 135008, 'temporary'),
      L('Accrued commissions — closing balance', 76832, 'temporary'),
    ],
